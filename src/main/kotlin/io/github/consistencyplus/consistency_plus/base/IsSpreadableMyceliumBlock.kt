...
interface IsSpreadableMyceliumBlock {
                ...
                for (i in 0..3) {
                    ...
                    if (world.getBlockState(targetPos).isOf(Blocks.DIRT) && customCanSpread(
                        ...
                        if (world.getBlockState(targetPos.up())
                            ...
                            world.setBlockState(targetPos, Blocks.MYCELIUM.defaultState.with(SnowyBlock.SNOWY, true))
                        } else {
                            world.setBlockState(targetPos, Blocks.MYCELIUM.defaultState)
                        }
                    }

                    // HasGrownGrassVariant handling
                    if (world.getBlockState(targetPos).block is HasGrownMyceliumVariant && customCanSpread(
                            world.getBlockState(
                                pos
                            ), world, targetPos
                        )
                    ) {
                        val targetBlock = world.getBlockState(targetPos).block as HasGrownMyceliumVariant
                        ...
                        if (world.getBlockState(targetPos.up())
                            ...
                            world.setBlockState(
                                targetPos, targetBlock.getGrownMyceliumVariant(world, targetPos)!!
