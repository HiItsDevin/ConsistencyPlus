...
class GrassStairsBlock(baseBlockState: BlockState?, settings: Settings?) :
    StairsBlock(baseBlockState, settings), HasUngrownVariant, IsSpreadableGrassBlock {
    ...
    override fun customCanSurvive(state: BlockState?, worldView: WorldView, pos: BlockPos): Boolean {
        ...
        return if (stateAbove.isOf(Blocks.SNOW) && stateAbove.get(SnowBlock.LAYERS) == 1) {
            ...
            if (worldView.getBlockState(pos).block is GrassStairsBlock) {
